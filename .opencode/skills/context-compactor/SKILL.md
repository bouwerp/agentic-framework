...
| **Warm** | Filesystem (`.plan/`, `NOTES.md`, `CLAUDE.md`) | 1 tool call | Yes | Unlimited |
...
3. Run /compact (Claude Code) or clear unnecessary context
...
Before compaction occurs (or before running `/compact`), write a checkpoint file that captures everything needed to resume:
...
| Nuanced user preferences | Agent reverts to defaults | Capture in CLAUDE.md or checkpoint |
...
- CLAUDE.md / AGENTS.md (re-injected automatically)
...
- **Use grep first** to locate what you need, then read the specific section.
...
- **Prefer `Edit` over `Write`** for changes — diffs are smaller than full files.
...
|-------------|-----------|-----|
| Reading entire files to find one function | 2,000-4,000 tokens | Grep first, then read the range |
...
| Not using `.claudeignore` / exclude patterns | 10,000+ tokens from irrelevant files | Exclude build artifacts, node_modules, lock files |
...
Proactive: Write findings to files every 2 research actions
           ...
           /compact between distinct phases
```
...
### Claude Code
- **`/compact`**: Manually triggers summarization. Achieves 60-70% token reduction.
- **`/compact focus on [topic]`**: Guides what the summary preserves.
- **Auto-compact**: Fires at ~85-95% capacity. Proactive `/compact` at ~70% produces better summaries.
- **`/clear`**: Clears context entirely. Use between unrelated tasks.
- **Post-compact hooks**: Configure in `.claude/settings.json` to re-inject critical context from a file after every compaction.
- **CLAUDE.md**: Re-loaded after every compaction — the only guaranteed survival mechanism. Keep essential rules here.

### Cursor
- Writes large tool outputs and chat history to files automatically.
- MCP tool descriptions loaded on-demand (not all upfront) to save context.
- Project Rules (`.cursor/rules/`) persist across sessions like CLAUDE.md.
- Keep code files under 500 lines for optimal agent consumption.

### OpenCode
- Context condensation replaces dropped events with LLM-generated summaries.
- Always preserves the first N events for task continuity.
- `.opencode/plans/` files persist across compaction.

### Pi
- Use AGENTS.md for persistent project instructions.
- Write session state to files — Pi's context is managed through its extension system.
- Skills as capability packages survive context resets since they're loaded from disk.

### Gemini
- Include "Read NOTES.md before proceeding" in system instructions.
- Gemini's context window varies by model — monitor usage actively.
- System instructions are re-injected every turn.

### Aider
- Repository map (tree-sitter) provides structural awareness without loading full files.
- Use `/drop` to remove inactive files from context.
- `--map-tokens` controls the repo map budget (default 1K tokens).
...
BETWEEN TASKS:
  /clear (full reset) — start fresh with a clean brief
