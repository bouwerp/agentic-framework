---
name: aws
description: This skill should be used when the user asks to interact with AWS services via the CLI — listing resources, checking logs, deploying infrastructure, managing S3, EC2, Lambda, ECS, CloudFormation, IAM, CloudWatch, RDS, DynamoDB, SQS, SNS, Secrets Manager, SSM, ECR, Route53, API Gateway, Step Functions, or Cost Explorer. Includes CDK/serverless/cost alignment summaries (with this repo’s CLI safety rules). Provides safe AWS CLI patterns with a read-first, confirm-before-mutating approach.
version: 1.1.0
---
...
## CDK, serverless, and cost (IaC alignment)

This section distils patterns from community AWS skill packs (e.g. [zxkane/aws-skills](https://github.com/zxkane/aws-skills)) while **keeping this skill’s CLI safety rules** (confirm before mutating, no casual IAM edits via CLI).
...
- **Lambda bundling:** Use language-appropriate constructs (`NodejsFunction`, Python Lambda patterns) instead of hand-rolled zip steps when possible.
- **Deploy:** Use change sets / pipeline flows for production; never bypass review for shared environments.

### Serverless and event-driven

- **Small, single-purpose functions**; design for **concurrency** and downstream throttling, not “total requests.”
- **Orchestration:** Prefer Step Functions (or event buses) over long synchronous Lambda chains.
- **State:** Treat local disk as ephemeral; persist in DynamoDB, S3, or queues as appropriate.
...
- **Estimate before create:** rough sizing for Lambda memory, NAT, always-on services, and data transfer.
- **Observe:** dashboards and alarms on errors, duration, throttles, queue age, and spend anomalies (Cost Explorer patterns above).
- **Optimize:** right-size, tagging for chargeback, lifecycle rules for logs and S3, remove idle resources — still subject to explicit user approval before destructive changes.
